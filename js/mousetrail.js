...
const initparticles1 = () => {
  for (var i = 0; i < nparticles1; i++) {
    particles1.push(new Particle(Math.random()*width, Math.random()*height));
    particles1[i].velocity.y = 0.1;
  }
}

const drawparticles1 = () => {
  for (var i = 0; i < nparticles1; i++) {
    particles1[i].update();
    particles1[i].follow();
    particles1[i].draw();
...
const launchParticle = () => {
  particles1[p].position.reset(emitter.x, emitter.y);
  particles1[p].velocity.reset(-1+ Math.random()*2, -1+Math.random()*2);
  particles1[p].color = `hsl(${Math.floor(emitter.x/width*256)},40%,${60+Math.random()*20}%)`;
  particles1[p].alpha = 1;
  p++;
  if (p === nparticles1) p = 0;
}
